...
> You should run `yarn rw build` before running this command.

`yarn rw serve` is useful for debugging locally or for self-hosting—deploying a single server into a serverfull environment. Since both the api and the web sides run in the same server, CORS isn't a problem.
